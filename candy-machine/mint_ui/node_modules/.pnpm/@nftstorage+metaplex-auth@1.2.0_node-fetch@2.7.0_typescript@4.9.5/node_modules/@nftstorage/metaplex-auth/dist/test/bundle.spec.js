...
describe('NFTBundle', () => {
    describe('addNFT', () => {
        it('adds an entry to the manifest', async () => {
            ...
            expect((_a = manifest['an-id']) === null || _a === void 0 ? void 0 : _a.encodedAssets.cid).to.eq(nft.encodedAssets.cid);
            expect((_b = manifest['an-id']) === null || _b === void 0 ? void 0 : _b.encodedMetadata.cid).to.eq(nft.encodedMetadata.cid);
        });
        it('fails to add entries with duplicate ids', async () => {
            ...
            await expect(bundle.addNFT('an-id', nft2.metadata, nft2.image)).to.be.rejectedWith('duplicate');
        });
        it('fails to add an entry with an id longer than MAX_ID_LEN', async () => {
            ...
            await expect(bundle.addNFT(id, metadata, image)).to.be.rejectedWith('length');
        });
        it('fails to add more than MAX_ENTRIES', async () => {
            ...
            try {
                ...
                const { metadata, image } = makeRandomNFT();
                await expect(bundle.addNFT('too-many', metadata, image)).to.be.rejectedWith(NFTBundle.MAX_ENTRIES.toString());
            }
            finally {
    ...
    describe('addNFTFromFileSystem', () => {
        ...
        it('assigns an id based on metadata filename if none is given', async () => {
            ...
            expect((_a = manifest['token']) === null || _a === void 0 ? void 0 : _a.encodedAssets.cid).to.eq(nft.encodedAssets.cid);
            expect((_b = manifest['token']) === null || _b === void 0 ? void 0 : _b.encodedMetadata.cid).to.eq(nft.encodedMetadata.cid);
        });
    });
...
function makeRandomNFT() {
    const image = new File(['pretend-this-is-image-data-' + Math.random()], 'image.png', {
        type: 'image/png',
    });
    const metadata = {
        image: 'image.png',
...
async function expectRootBlockToHaveCIDs(rootBlock, blockstore, metadataCIDs, assetCIDs) {
    ...
    for (const link of links) {
        ...
        expect(metadataCIDs.some((cid) => cid.equals(metadataLink === null || metadataLink === void 0 ? void 0 : metadataLink.Hash))).to.be
            .true;
        expect(assetCIDs.some((cid) => cid.equals(assetsLink === null || assetsLink === void 0 ? void 0 : assetsLink.Hash))).to.be.true;
    }
}
