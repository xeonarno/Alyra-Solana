...
export class NFTBundle {
    ...
    _enforceMaxEntries() {
        if (Object.keys(this._nfts).length >= NFTBundle.MAX_ENTRIES) {
            throw new Error(`unable to add more than ${NFTBundle.MAX_ENTRIES} to a bundle.`);
        }
    }
    _enforceMaxIdLength(id) {
        const len = new TextEncoder().encode(id).byteLength;
        if (len > NFTBundle.MAX_ID_LEN) {
            throw new Error(`NFT id exceeds max length (${NFTBundle.MAX_ID_LEN} bytes): ${id}`);
        }
    }
...
async function wrapperDirForNFT(nft) {
    ...
    const metadataLink = dagPb.createLink('metadata', metadataBlock.bytes.byteLength, nft.encodedMetadata.cid);
    const assetsLink = dagPb.createLink('assets', assetsBlock.bytes.byteLength, nft.encodedAssets.cid);
